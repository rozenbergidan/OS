...
enum sched_priority get_highest_priority()
{
    ...
    for (i = 0; i < MAX_UTHREADS; i++)
    {
        if (threads[i].state == UT_RUNNABLE && threads[i].priority > max_priority)
        {
...
void uthread_sched(struct uthread *current)
{
    ...
    while (next_thread->state != UT_RUNNABLE || next_thread->priority != max_priority)
    {
...
void uthread_exit()
{
    uthread_self()->state = UT_FREE;

    int i;
    int flag = 0;
    for (i = 0; i < MAX_UTHREADS; i++)
    {
        if (threads[i].state != UT_FREE)
            flag = 1;
    }
    if (flag == 0)
    {
