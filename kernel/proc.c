...
void procinit(void)
{
  ...
  for (p = proc; p < &proc[NPROC]; p++)
  {
    ...
    initlock(&p->kthread_counter_lock, "kthread_counter_lock");
    p->kthread_counter = 0;
    p->state = UNUSED;
    kthreadinit(p);
  }
...
myproc(void)
{
  push_off();
  struct cpu *c = mycpu();
  struct proc *p = c->thread->kthread_parent;
  pop_off();
  return p;
}
...
void exit(int status)
{
  ...
  for (kt = p->kthread; kt < &p->kthread[NKT]; kt++)
  {
    ...
    kt->kthread_xstate = status;
    kt->kthread_state = ZOMBIE;
    release(&kt->kthread_lock);
  }
  p->xstate = status;
  p->state = ZOMBIE;
  // release(&p->kthread[0].kthread_lock); DOTO: should we release here?
  release(&wait_lock);
